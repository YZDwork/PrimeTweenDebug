...
public partial class Tests {
    ...
    static int tweensCapacity {
        get {
            var m = PrimeTweenManager.Instance;
            var result = m.currentPoolCapacity;
            Assert.AreEqual(result, m.tweens.Capacity);
            return result;
        }
    }
    ...
    public IEnumerator Elastic() {
        for (int i = 0; i < 5; i++) {
            ...
            Assert.AreEqual(period, t.tween.settings.parametricEasePeriod); // todo why test fails??
